...
{	
	...
	{	      
		...
		if(Usart_Receive==0x59)  sudu=2;  //低速挡（默认值）
		if(Usart_Receive==0x58)  sudu=1;  //高速档 
	 if(Usart_Receive==0x4B) Turn_Flag=1;  //进入转向控制界面
		...
		if(Turn_Flag==0)
		{
			...
			else if(Usart_Receive==0x42)	Flag_Qian=1,Flag_Hou=0,Flag_Left=0,Flag_Right=1;//////////////前右			
			else if(Usart_Receive==0x43)	Flag_Qian=0,Flag_Hou=0,Flag_Left=0,Flag_Right=1;  //左
			else if(Usart_Receive==0x44)	Flag_Qian=0,Flag_Hou=1,Flag_Left=0,Flag_Right=1;//////////////后右	
		  else if(Usart_Receive==0x45)	Flag_Qian=0,Flag_Hou=1,Flag_Left=0,Flag_Right=0;//////////////后
		  else if(Usart_Receive==0x46)	Flag_Qian=0,Flag_Hou=1,Flag_Left=1,Flag_Right=0;//////////////左后
			else if(Usart_Receive==0x47)	Flag_Qian=0,Flag_Hou=0,Flag_Left=1,Flag_Right=0;
	    else if(Usart_Receive==0x48)	Flag_Qian=1,Flag_Hou=0,Flag_Left=1,Flag_Right=0;//左前
			else Flag_Qian=0,Flag_Hou=0,Flag_Left=0,Flag_Right=0;//////////////刹车 
		}
		else	 if(Turn_Flag==1)//如果进入了转向控制界面
		{
		     if(Usart_Receive==0x43) Turn_Left=0,Turn_Right=1;    
				 else if(Usart_Receive==0x47) Turn_Left=1,Turn_Right=0;
				 else Turn_Left=0,Turn_Right=0;
		}
		//以下是与APP调试界面通讯
